...
for (var aWireCount = 1; aWireCount <= 1000; aWireCount++)
{
    // Kleinste Distanz herausfinden
    var aMinDistance = aDistances.Min(kvp => kvp.Value);
...
// Absteigend nach Anzahl der Boxen sortieren
aCircuts = aCircuts.OrderByDescending(c => c.Count).ToList();

var aProduct = aCircuts[0].Count * aCircuts[1].Count * aCircuts[2].Count;
